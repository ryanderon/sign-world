...
export default function Services() {
  ...
  const services = [
    { id: "custom-signage", icon: serviceIcons.customSignage, translationKey: "customSignage" },
    { id: "huruf-timbul", icon: serviceIcons.hurufTimbul, translationKey: "3dLettering" },
    { id: "signage-akrilik", icon: serviceIcons.signageAkrilik, translationKey: "acrylicSignage" },
    { id: "neon-box", icon: serviceIcons.neonBox, translationKey: "ledNeonBox" },
    { id: "visual-branding", icon: serviceIcons.visualBranding, translationKey: "visualBranding" },
    { id: "display-akrilik", icon: serviceIcons.displayAkrilik, translationKey: "acrylicDisplays" },
  ];
      ...
      <div className="py-20 md:py-28 bg-surface">
        <div className="container">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <span className="text-subheading mb-4 block">{t("services.sectionTitle")}</span>
              ...
              <div className="space-y-4">
                {whyChooseUs.map((item) => (
                  <div key={item.translationKey} className="flex items-start gap-3">
            ...
            <div className="relative hidden lg:block">
              ...
              <div className="absolute -bottom-6 -right-6 bg-white rounded-xl shadow-xl p-5 border border-border max-w-[200px]">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-text-muted">{t("services.materialOptions")}</span>
                </div>
                <p className="text-xs text-text-muted">{t("services.materialOptionsDesc")}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div id="services" className="py-20 md:py-28">
        <div className="container">
          <SectionHeader
            ...
            heading={<>{t("services.expertiseHeading")} <span className="text-gradient-warm">{t("services.expertiseHighlight")}</span></>}
            ...
            {services.map((service) => (
              <article key={service.id} className="service-card p-6">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {(t(`services.items.${service.translationKey}.features`, { returnObjects: true }) as string[]).map((feature) => (
                    <span key={feature} className="text-xs px-2.5 py-1 bg-surface rounded-md text-text-muted">
                      {feature}
                    </span>
                  ))}
                </div>

                <a href="#contact" className="inline-flex items-center gap-2 text-sm font-semibold text-teal hover:text-blue transition-colors">
      ...
      <div id="process" className="py-20 md:py-28 bg-surface">
        <div className="container">
          <SectionHeader
            ...
            heading={<>{t("process.heading")} <span className="text-gradient">{t("process.headingHighlight")}</span></>}
          ...
          <div className="relative rounded-3xl overflow-hidden p-10 md:p-16" style={{ background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f172a 100%)' }}>
            ...
            <div className="relative grid md:grid-cols-2 gap-8 items-center">
              ...
              <div className="flex flex-wrap gap-4 md:justify-end">
                ...
                <a href="tel:+6282200000000" className="btn-ghost text-white border border-white/20 hover:bg-white/10 rounded-full">
