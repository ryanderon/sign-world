...
const serviceCategories = [
  {
    ...
    textColor: "text-teal",
    image: importImages("signage", "1.jpeg"),
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
  ...
  },
  {
    ...
    image: importImages("printing", "8.jpeg"),
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
  ...
  },
  {
    ...
    image: importImages("merchandise", "5.jpeg"),
    icon: (
      <svg
        className="w-6 h-6"
        fill="none"
  ...
  {
    id: "printingInstallation",
    translationKey: "printingInstallation",
    href: "/printing-installation",
    isExternal: false,
    gradient: "from-green to-teal",
    bgGradient: "from-green/5 to-teal/5",
...
interface MoreServicesProps {
  currentPage?: "signage" | "printing" | "printingInstallation" | "merchandise";
}
    ...
    <section ref={ref} className="py-20 md:py-28 bg-surface">
      <div className="container">
        <SectionHeader
          ...
          heading={
            <>
              {t("moreServices.heading")}{" "}
              <span className="text-gradient-warm">
                {t("moreServices.headingHighlight")}
              </span>
            </>
          }
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {serviceCategories.map((category) => {
            ...
            const CardContent = (
              <article
                className={`group relative bg-white rounded-2xl overflow-hidden shadow-lg transition-all duration-500 border ${
                  category.borderColor
                } h-full flex flex-col ${
                  featured
                    ? "ring-2 ring-teal/30 hover:shadow-2xl"
                    : current
                    ? "ring-2 ring-primary/30 opacity-75"
                    : "hover:shadow-2xl"
                }`}
              >
                {featured && (
                ...
                <div className="relative h-48 overflow-hidden flex-shrink-0">
                  <img
                    src={category.image}
                    alt={t(
                      `moreServices.items.${category.translationKey}.title`
                    )}
                  ...
                  />
                  <div
                    className={`absolute inset-0 bg-gradient-to-t ${category.gradient} opacity-20`}
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />

                  <div className="absolute bottom-4 left-4">
                    <div
                      className={`w-12 h-12 rounded-xl bg-gradient-to-br ${category.gradient} flex items-center justify-center text-white shadow-lg`}
                    >
                      {category.icon}
                    </div>
                  </div>
                ...
                <div
                  className={`p-6 bg-gradient-to-br ${category.bgGradient} flex flex-col flex-1`}
                >
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {(
                      t(`moreServices.items.${category.translationKey}.tags`, {
                        returnObjects: true,
                      }) as string[]
                    ).map((tag) => (
                      <span
                  ...
                  <div
                    className={`inline-flex items-center gap-2 text-sm font-semibold ${
                      ...
                      current ? "" : "group-hover:gap-3"
                    } transition-all mt-auto`}
                  >
                    {current
                      ? t("moreServices.youreHere")
                      : t(`moreServices.items.${category.translationKey}.cta`)}
            ...
            if (current) {
              return (
                <div key={category.id} className="cursor-default">
                  {CardContent}
                </div>
              );
            }
